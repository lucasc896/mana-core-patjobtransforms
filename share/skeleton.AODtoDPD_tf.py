...
if type(dpdPreInclude) is not list:
    ...
    additionalOutputs = set( [ outfile for outfile in b if outfile.startswith('output') and outfile.endswith('File') ] ) - set(dpdPreInclude)
...
if type(dpdPostIncludeUsed) is not list:
    ...
    additionalOutputs = set( [ outfile for outfile in b if outfile.startswith('output') and outfile.endswith('File') ] ) - set(dpdPostIncludeUsed)
    if len(additionalOutputs) > 0:
